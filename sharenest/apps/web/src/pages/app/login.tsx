...
export default function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // リダイレクト先がある場合はそこに、なければ車両一覧に
      const redirectTo = router.query.redirect as string || '/app/vehicles';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="absolute bottom-20 right-20 w-32 h-32 bg-cyan-400/20 rounded-full blur-2xl animate-[float_8s_ease-in-out_infinite_reverse]"></div>
      
      <div className="max-w-md w-full space-y-8 glass rounded-3xl p-10 shadow-2xl backdrop-blur-xl">
        <div className="text-center">
          <div className="text-5xl mb-4">🚗</div>
          <h2 className="text-center text-4xl font-black gradient-text-cyber mb-2">Welcome Back</h2>
          <p className="text-xl text-white/80 font-light">京都・大阪のプレミアムカーシェア</p>
          <p className="mt-4 text-center text-sm text-white/70">
            アカウントをお持ちでない方は{' '}
            <Link href="/app/register" className="font-medium text-cyan-300 hover:text-cyan-200 transition-colors">
              新規登録
            </Link>
          </p>
        </div>
        <form className="mt-8 space-y-6" onSubmit={onSubmit}>
          <div className="space-y-6">
            <div>
              ...
              <input
                ...
                type="email"
                required
                className="relative block w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl placeholder-white/60 text-white focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-transparent transition-all duration-300 backdrop-blur-sm"
          ...
          <div>
            <button
              type="submit"
              disabled={loading}
              className="group relative w-full flex justify-center py-4 px-4 border border-transparent text-lg font-bold rounded-xl text-white bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-cyan-500 hover:to-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-400 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-lg hover:shadow-cyan-500/25"
            >
              {loading ? (
